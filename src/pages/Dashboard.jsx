...
function PostContent({ item }) {
  ...
  return (
    <>
      ...
      {item.imageUrl && (
        <div className="post-image-container" style={{ marginBottom: 16, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border-color)' }}>
          <img 
            src={item.imageUrl.startsWith('http') ? item.imageUrl : `${api.defaults.baseURL}${item.imageUrl}`} 
            alt="Post content" 
